...
public class SpringJdbcTemplateMain {
	...
	public static void main(String[] args) {
		...
		/*
		 ...
		 ResultSet으로부터 데이터를 컬럼이름으로 get해서 
		 Guest객체의 테이블컬럼이름과동일한이름의 property(멤버변수)에 대입(setter method호출)하는 객체
		 */
		BeanPropertyRowMapper<Guest> guestBeanPropertyRowMapper=
				new BeanPropertyRowMapper<Guest>(Guest.class);
		
		Guest guest = 
				jdbcTemplate.queryForObject(selectDtoSql,new Object[]{1},guestBeanPropertyRowMapper);
		...
		System.out.println("queryForList[List<Guest>] : "+mapList);
		*/
		/*
		 queryForObject [반환타입 String]
		 */
		String selectStringSql="select guest_name from guest where guest_no=?";
		String guest_name=jdbcTemplate.queryForObject(selectStringSql, new Object[] {1},String.class);
		...
		System.out.println("queryForList[List<GUest>] : "+guest_nameList);
		
		/*
		 queryForObject [반환타입 Integer]
		 */
		String selectIntegerSql="select guest_no from guest where guest_no=?";
		int guest_no=jdbcTemplate.queryForObject(selectIntegerSql, new Object[] {1},Integer.class);
		System.out.println("queryForObject[String]"+guest_no);
		
		/*
		 queryForList [반환타입 Integer]
		 */
